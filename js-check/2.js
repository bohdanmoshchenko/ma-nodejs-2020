function checkObject(object) {
    ...
    if (!Array.isArray(phones) || !phones) {
        ...
        phones.forEach(element => {
            if (!checkType(element.type, STRING_TYPE) || !Object.values(PhoneNumberType).includes(element.type)) {
                writeToConsole(element.type + IS_NOT_A + STRING_TYPE + ' or not a ' + PhoneNumberType);
            }
            let regexPatern = /\(\d\d\d\)\s\d\d\d-\d\d\d\d/;
            if (!checkType(element.number, STRING_TYPE) || !regexPatern.test(element.number)) {
                writeToConsole(element.type + IS_NOT_A + STRING_TYPE + ' or not a ' + PhoneNumberType);
            }
